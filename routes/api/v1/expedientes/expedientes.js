...
router.post('/new', async(req, res) => {
    const { identidad, fecha, descripcion, registros, ultimaActualizacion } = req.body;
    try {
        rslt = await expedienteModel.new(identidad, fecha, descripcion, registros, ultimaActualizacion);
        res.status(200).json({
            status: 'ok',
            result: rslt
        });
...
router.put('/update/:id', async(req, res) => {
    try {
        const { identidad, fecha, descripcion, registros, ultimaActualizacion } = req.body;
        const { id } = req.params;
        const result = await expedienteModel.updateOne(id, identidad, fecha, descripcion, registros, ultimaActualizacion);
        res.status(200).json({
            status: 'ok',
            result
        });
